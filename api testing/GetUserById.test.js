...
test('GET /v1/my-guard-users/users/:id should throw 401 when token is invalid', async () => {
...
test('GET /v1/my-guard-users/users/:id should throw 401 when token is not exist', async () => {
  await axios.get(`${url}/${config.testUser.id}`)
    .catch((error) => expect(error.response.status).toEqual(401));
});

test('GET /v1/my-guard-users/users/:id should throw an error message with the correct status property', async () => {
  await axios.get(`${url}/${config.testUser.id}`)
    .catch((error) => { 
...
test('GET /v1/my-guard-users/users/:id should throw 404 when an incorrect URL', async () => {
